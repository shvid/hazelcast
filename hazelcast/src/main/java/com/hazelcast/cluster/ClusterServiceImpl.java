...
//                    final boolean destroyExistingConnection = existingConnection != conn;
    ...
    private void fireMemberAttributeEvent(final MemberImpl member, final MapOperationType operationType, String key, Object value) {
        final MemberAttributeEvent membershipEvent = new MemberAttributeEvent(getClusterProxy(), member, 
                  Collections.unmodifiableSet(new HashSet<Member>(getMemberList())), operationType, key, value);
        final Collection<MembershipAwareService> membershipAwareServices = nodeEngine.getServices(MembershipAwareService.class);
        if (membershipAwareServices != null && !membershipAwareServices.isEmpty()) {
            final MemberAttributeServiceEvent event = new MemberAttributeServiceEvent(getClusterProxy(), member, 
                  Collections.unmodifiableSet(new HashSet<Member>(getMemberList())), operationType, key, value);
            for (final MembershipAwareService service : membershipAwareServices) {
                // service events should not block each other
                nodeEngine.getExecutionService().execute(ExecutionService.SYSTEM_EXECUTOR, new Runnable() {
